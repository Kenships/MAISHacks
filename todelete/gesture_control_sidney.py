...
class GestureController:
    def __init__(self, callback,  show_video=True):
    ...
    def run(self):
        ...
        controller = MainController("dynamic_gestures/models/hand_detector.onnx", "dynamic_gestures/models/crops_classifier.onnx")
        drawer = Drawer()
        debug_mode = True
        while cap.isOpened():
            ret, frame = cap.read()
            ...
            if ret:
                    ...
                    for trk in controller.tracks:
                        if trk["tracker"].time_since_update < 1:
                            if len(trk['hands']):
                                ...
                                self.gesture_name = targets[current_gesture] if current_gesture is not None else "None"
                ...
                if self.gesture_name != "None":
                    self.callback('gesture', self.gesture_name, trk['hands'][-1])
                    self.gesture_name = "None"
