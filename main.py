...
while running:
    ...
    # Clamp + cast color to ints
    color_int = tuple(max(0, min(255, int(c))) for c in poly_color)

    # Cast polygon vertices to ints (pygame is happier with ints)
    poly_int = [(int(p[0]), int(p[1])) for p in poly]

    # Only draw if there are at least 3 points
    if len(poly_int) >= 3:
        pygame.draw.polygon(screen, color_int, poly_int)
